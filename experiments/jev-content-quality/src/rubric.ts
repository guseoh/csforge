...
export function buildRubric(kind: CandidateKind, language: InstructionLanguage): RubricDefinition {
  ...
  const questions: Record<string, RubricQuestion> = kind === "QUESTION"
    ? korean
      ? {
          ...
          multiple_defensible_answers: noul("state의 prompt, choices, expected answer를 함께 볼 때 서로 다른 답이 모두 defensible한가?", "둘 이상의 답이 주어진 조건에서 방어 가능하다.", "하나의 답이 주어진 조건에서 방어 가능하다."),
          answer_explanation_conflict: noul("state의 expected answer와 explanation이 실질적으로 서로 충돌하는가?", "정답과 해설이 함께 참일 수 없다.", "정답과 해설이 양립한다."),
          linked_concept_misalignment: noul("state의 Question이 linked Concept의 title/summary가 선언하는 learning objective와 어긋나는가?", "질문이 linked Concept의 핵심 목표를 평가하지 않는다.", "질문이 linked Concept의 목표를 평가한다."),
          ...
          weak_distractor: noul("state가 MULTIPLE_CHOICE라면 distractor가 지나치게 무관하거나 정답을 노출할 정도로 약한가?", "하나 이상의 distractor가 같은 주제의 plausible misconception이 아니다.", "distractor가 같은 주제의 plausible misconception이다."),
      ...
      : {
          ...
          multiple_defensible_answers: noul("Given the prompt, choices, and expected answer in the state, are multiple answers defensible?", "More than one answer is defensible under the stated conditions.", "One answer is defensible under the stated conditions."),
          ...
          linked_concept_misalignment: noul("Does the Question fail to assess the learning objective declared by the linked Concept title and summary?", "The Question misses the linked Concept's core objective.", "The Question assesses the linked Concept's objective."),
          ...
          weak_distractor: noul("If the state is MULTIPLE_CHOICE, is any distractor irrelevant or so weak that it exposes the answer?", "At least one distractor is not a plausible misconception from the same topic.", "The distractors are plausible misconceptions from the same topic."),
