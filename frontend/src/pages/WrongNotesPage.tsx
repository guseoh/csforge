...
import type { WrongNoteSearch } from '../lib/wrong-note-search'
...
export function WrongNotesPage() {
  ...
  const update = (key: keyof WrongNoteSearch, value: string) => void navigate({
    search: (current) => ({ ...current, [key]: value, page: 0 }),
  })
  const updateArea = (value: string) => void navigate({
    search: (current) => ({ ...current, area: value, topic: '', page: 0 }),
  })
  ...
  return (
    <section className="page-section">
      ...
      <div className="filter-panel wrong-note-filters">
        ...
        <label>Topic
          <select value={search.topic} onChange={(event) => update('topic', event.target.value)} disabled={!search.area || topicsQuery.isPending || topicsQuery.isError}>
            <option value="">All topics</option>
            {topicsQuery.data?.topics.map((topic) => <option key={topic.id} value={String(topic.id)}>{topic.title}</option>)}
          </select>
        </label>
        <label>Level
          <select value={search.level} onChange={(event) => update('level', event.target.value)}>
            <option value="">All levels</option><option value="1">Level 1</option><option value="2">Level 2</option><option value="3">Level 3</option>
          </select>
        </label>
        <label>Difficulty
          <select value={search.difficulty} onChange={(event) => update('difficulty', event.target.value)}>
        ...
        <label>Status
          <select value={search.status} onChange={(event) => update('status', event.target.value)}><option value="">All</option><option value="ACTIVE">Active</option><option value="MASTERED">Mastered</option></select>
        </label>
        <label>Review
          <select value={search.review} onChange={(event) => update('review', event.target.value)}><option value="ALL">All</option><option value="DUE">Due</option><option value="SCHEDULED">Scheduled</option><option value="MASTERED">Mastered</option><option value="NONE">No schedule</option></select>
        </label>
        <label>AI analysis
          <select value={search.analysis} onChange={(event) => update('analysis', event.target.value)}><option value="ALL">All</option><option value="NOT_REQUESTED">Not requested</option><option value="PENDING">Pending</option><option value="PROCESSING">Processing</option><option value="COMPLETED">Completed</option><option value="FAILED">Failed</option></select>
        </label>
        <label>Sort
          <select value={search.sort} onChange={(event) => update('sort', event.target.value)}><option value="RECENT">Recent</option><option value="WRONG_COUNT">Wrong count</option><option value="REVIEW_DUE">Review due</option></select>
        </label>
      </div>
