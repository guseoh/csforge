...
export function WrongNoteDetailPage() {
  ...
  return (
    <section className="page-section wrong-note-detail">
      ...
      <section className="detail-section study-section"><div className="section-heading"><div><p className="eyebrow">최근 풀이</p><h2>최근 오답</h2></div>{item.latestWrongAttempt && <span className="chip state-badge state-active">{gradingStatusLabels[item.latestWrongAttempt.gradingStatus] ?? item.latestWrongAttempt.gradingStatus}</span>}</div><p className="latest-answer">{item.latestWrongAttempt?.answerText ?? item.latestWrongAttempt?.selectedChoiceKey ?? '답변하지 않음'}</p><p className="helper-text">{item.latestWrongAttempt ? `${sourceLabel(item.latestWrongAttempt.source)} · ${item.latestWrongAttempt.answeredAt ? new Date(item.latestWrongAttempt.answeredAt).toLocaleString('ko-KR') : '답변 시간 없음'}` : '최근 오답 기록이 없습니다.'}</p></section>
      <section className="detail-section study-section"><p className="eyebrow">정답 확인</p><h2>정답과 해설</h2><div className="answer-facts"><p><strong>정답 선택지:</strong> {item.answer.correctChoiceKey ?? '—'}</p><p><strong>허용 답안:</strong> {item.answer.acceptedAnswers.join(', ') || '—'}</p></div>{item.answer.modelAnswer ? <MarkdownContent className="answer-block">{item.answer.modelAnswer}</MarkdownContent> : <p className="helper-text">모범 답안이 없습니다.</p>}{item.question.explanationMarkdown ? <MarkdownContent className="answer-block">{item.question.explanationMarkdown}</MarkdownContent> : <p className="helper-text">설명이 없습니다.</p>}</section>
      ...
      <section className="detail-section related-learning-section">
        <div className="section-heading"><div><p className="eyebrow">이어 학습하기</p><h2>관련 개념</h2></div><span className="helper-text">틀린 문제와 연결된 개념을 다시 확인해 보세요.</span></div>
        <ConceptContext concepts={item.concepts} />
      </section>
      {aiAvailable && <section className="detail-section ai-analysis-section">
