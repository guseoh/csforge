  ...
  return (
    <section className="page-section wrong-note-detail">
      ...
      <section className="detail-section">
        ...
        {attemptsQuery.hasNextPage && <div className="load-more-row"><button className="secondary-button" type="button" disabled={attemptsQuery.isFetchingNextPage} onClick={() => void attemptsQuery.fetchNextPage()}>{attemptsQuery.isFetchingNextPage ? '불러오는 중…' : '더 불러오기'}</button>{attemptsQuery.isFetchNextPageError && <><span className="helper-text error-text">추가 기록을 불러오지 못했습니다.</span><button className="text-button" type="button" onClick={() => void attemptsQuery.fetchNextPage()}>다시 시도</button></>}</div>}
      </section>
    </section>
  )
}
