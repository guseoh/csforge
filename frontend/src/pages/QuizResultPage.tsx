...
export function QuizResultPage() {
  ...
  return (
    <section className="page-section quiz-page">
      <div className="page-heading"><div><p className="eyebrow">Quiz result · {result.source.replace('_', ' ')}</p><h1>Review your run.</h1><p className="lead">정답과 해설을 확인하고, 자기채점 문항을 마무리하세요.</p></div><Link className="secondary-button" to="/quiz" search={defaultQuizSearch}>새 Quiz</Link></div>
      ...
      <div className="quiz-result-actions"><button className="primary-button" type="button" disabled={result.selfCheckPending > 0 || result.wrong + result.unanswered === 0 || retryMutation.isPending} onClick={() => retryMutation.mutate()}>{retryMutation.isPending ? '준비 중…' : '틀린 문항만 다시 풀기'}</button><Link className="secondary-button" to="/review" search={{ page: 0, due: 'ALL' }}>복습 일정 보기</Link>{result.selfCheckPending > 0 && <span className="helper-text">Retry 전에 self-check를 모두 완료하세요.</span>}{retryMutation.isError && <span className="helper-text error-text">Retry를 시작하지 못했습니다.</span>}</div>
    </section>
  )
}
