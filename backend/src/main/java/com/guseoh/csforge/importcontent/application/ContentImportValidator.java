...
public class ContentImportValidator {
    public List<NormalizedImportItem> validate(List<NormalizedImportItem> items) {
        ...
        items.stream()
                .filter(item -> item.kind() == ImportItemKind.TOPIC && !item.isError())
                .map(NormalizedImportItem::contentKey)
                .forEach(topics::add);
        items.stream()
                .filter(item -> item.kind() == ImportItemKind.CONCEPT && !item.isError())
                .map(NormalizedImportItem::contentKey)
                .forEach(concepts::add);
        Map<String, NormalizedImportItem> definitions = new HashMap<>();
        Map<String, NormalizedImportItem> topicSlugs = new HashMap<>();
        Map<String, NormalizedImportItem> conceptSlugs = new HashMap<>();
        ...
        for (NormalizedImportItem item : items) {
            ...
            if (item.contentKey() != null) {
                ...
                if (previous != null && !previous.sameCanonicalPayload(item)) {
                    errors.add(new ImportValidationError("contentKey", "충돌하는 contentKey가 같은 batch에 중복되었습니다"));
                }
            }
            if (item.kind() == ImportItemKind.TOPIC && item.areaSlug() != null && item.slug() != null) {
                NormalizedImportItem previousSlug = topicSlugs.putIfAbsent(item.areaSlug() + "\u0000" + item.slug(), item);
                if (previousSlug != null && !previousSlug.contentKey().equals(item.contentKey())) {
                    errors.add(new ImportValidationError("slug", "같은 LearningArea에 동일한 slug가 batch에 중복되었습니다"));
                }
            }
            if (item.kind() == ImportItemKind.CONCEPT && item.topicContentKey() != null && item.slug() != null) {
                NormalizedImportItem previousSlug = conceptSlugs.putIfAbsent(item.topicContentKey() + "\u0000" + item.slug(), item);
                if (previousSlug != null && !previousSlug.contentKey().equals(item.contentKey())) {
                    errors.add(new ImportValidationError("slug", "같은 Topic에 동일한 slug가 batch에 중복되었습니다"));
                }
            }
            if (item.kind() == ImportItemKind.TOPIC) {
                ...
                validateNonNegative(item.displayOrder(), "displayOrder", errors);
            } else if (item.kind() == ImportItemKind.QUESTION) {
                validateQuestion(item, concepts, errors);
            }
            if (item.kind() == ImportItemKind.TOPIC) {
                validateNonNegative(item.displayOrder(), "displayOrder", errors);
            }
            for (NormalizedReference ref : item.references()) {
                try {
                    ReferenceType.valueOf(ref.referenceType());
                } catch (Exception e) {
                    errors.add(new ImportValidationError("references.referenceType", "지원하지 않는 referenceType입니다"));
                }
                validateNonNegative(ref.displayOrder(), "references.displayOrder", errors);
            }
    ...
    private void validateQuestion(NormalizedImportItem item, Set<String> concepts, List<ImportValidationError> errors) {
        ...
        try {
            type = QuestionType.valueOf(item.questionType());
        } catch (Exception e) {
            errors.add(new ImportValidationError("questionType", "지원하지 않는 questionType입니다"));
        }
        try {
            QuestionDifficulty.valueOf(item.difficulty());
        } catch (Exception e) {
            errors.add(new ImportValidationError("difficulty", "지원하지 않는 difficulty입니다"));
        }
        validateStatus(item.status(), errors);
        ...
        for (NormalizedChoice choice : item.choices()) {
            if (!choiceKeys.add(choice.key())) {
                errors.add(new ImportValidationError("choices.key", "choice key가 중복되었습니다"));
            }
            if (!orders.add(choice.displayOrder())) {
                errors.add(new ImportValidationError("choices.displayOrder", "choice displayOrder가 중복되었습니다"));
            }
        }
        Set<String> acceptedAnswers = new HashSet<>();
        for (String answer : item.acceptedAnswers()) {
            if (!acceptedAnswers.add(answer.toLowerCase(Locale.ROOT))) {
                errors.add(new ImportValidationError("acceptedAnswers", "accepted answer가 중복되었습니다"));
            }
        }
        try {
        ...
        if ("PUBLISHED".equals(item.status()) && type != null) {
            switch (type) {
                case MULTIPLE_CHOICE -> validatePublishedMultipleChoice(item, choiceKeys, errors);
                case SHORT_ANSWER -> {
                ...
                case DESCRIPTIVE, SCENARIO -> {
                    if (item.modelAnswer() == null) {
                        errors.add(new ImportValidationError("modelAnswer", "공개 서술형은 모범 답안이 필요합니다"));
                    }
                }
            }
        }
    ...
    private static void validatePublishedMultipleChoice(
            NormalizedImportItem item,
            Set<String> choiceKeys,
            List<ImportValidationError> errors) {
        if (item.choices().size() < 2) {
            errors.add(new ImportValidationError("choices", "공개 객관식은 선택지 2개 이상이 필요합니다"));
        }
    ...
    private static void validateStatus(String status, List<ImportValidationError> errors) {
        try {
            ...
            errors.add(new ImportValidationError("status", "지원하지 않는 status입니다"));
        }
    }

    private static void validateNonNegative(int value, String path, List<ImportValidationError> errors) {
        if (value < 0) errors.add(new ImportValidationError(path, "0 이상이어야 합니다"));
    }

    private static NormalizedImportItem copyWithErrors(
            NormalizedImportItem item,
            List<ImportValidationError> errors) {
