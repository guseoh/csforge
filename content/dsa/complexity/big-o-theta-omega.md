...
상수와 낮은 차수 항을 버리는 표기는 서로 다른 구현의 작은 입력 성능을 설명하지 않는다. `O(n)` 알고리즘도 큰 상수와 cache miss가 있으면 `O(n log n)` 구현보다 느릴 수 있고, 표기만으로 memory와 지연 시간을 모두 설명할 수 없다.

### Backend 연결

page API의 limit, search index의 후보 수, batch query의 반복 횟수를 각각 n으로 모델링한다. SLA 판단에는 점근식과 함께 실제 상한·분포·storage 지연 시간을 기록한다.
