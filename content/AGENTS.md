...
### Language / Terminology contract

학습 본문, 문제, 해설, 선택지, 표 제목과 설명용 라벨은 기술 용어·고유 명칭·코드·식별자·프로토콜 필드가 아니라면 자연스러운 한국어를 기본으로 한다. 한국 개발 현장에서 영어 자체로 통용되는 핵심 기술 용어는 학습에 도움이 될 때 첫 등장에 `한글(English)`로 연결하고, 이후에는 문맥에 맞는 한 표현을 사용한다.

JVM, JMM, GC, happens-before, CAS, Spring Bean, MVCC, HTTP 헤더와 메서드, SQL 키워드, Java API처럼 정확한 이름이 중요한 표현은 유지한다. 반면 일반 설명에서 쓰는 request/response, failure, fallback, latency, throughput, validation 같은 말은 특별히 영어를 유지할 이유가 없으면 요청/응답, 실패·장애, 대체 처리, 지연 시간, 처리량, 검증처럼 쓴다. 번역 때문에 서로 다른 기술 개념의 경계가 흐려지거나 문장이 더 어려워지면 정확한 원어를 유지한다.
