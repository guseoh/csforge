...
이 흐름에는 backpressure도 있다. receiver가 느리거나 network가 막히면 send buffer가 차고 sender의 blocking call이 기다리거나 non-blocking call이 partial result·retry 상태를 반환할 수 있다. connection close, reset, timeout은 서로 다른 실패 관찰일 수 있으므로 socket을 단순한 무한 byte pipe로 모델링하면 안 된다.
...
또한 timeout 뒤 응답이 보이지 않는다고 해서 server가 요청을 처리하지 않았다고 단정할 수 없다. 요청이 server에 도착해 처리된 뒤 응답만 유실됐을 수도 있기 때문이다. retry를 설계한다면 요청 id, duplicate detection, idempotent operation 또는 결과 조회를 함께 정의해야 한다.

### Local IPC에서 remote service로 경계가 바뀐다

loopback을 포함한 network socket은 Unix-domain socket보다 host 경계를 확장하기 쉽지만 network 실패, address reachability, TLS/authentication 같은 추가 책임이 생긴다. 반대로 Unix-domain socket은 같은 host라는 범위와 local permission/credential을 활용할 수 있다. socket API가 비슷하다는 이유로 두 방식의 실패 경계와 security boundary가 같다고 보지 않는다.

Backend service 호출에서도 `request bytes가 local send buffer에 accepted됨`, `peer가 response를 보냄`, `HTTP status를 받음`, `business transaction이 commit됨`을 별도 state로 둔다. 요청 id와 application idempotency key가 있어야 transport timeout 뒤 재시도가 중복 effect를 만들지 않도록 제어할 수 있다.
