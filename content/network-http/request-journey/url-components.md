...
### URL과 전송 대상
    scheme → protocol/TLS
    authority → host + port
    path/query → HTTP 요청-target
    fragment → user-agent local state (usually not sent)
fragment를 server routing key로 가정하면 요청에서 값이 사라진다.
