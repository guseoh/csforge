...
UDP를 선택한다고 지연 시간이 항상 낮아지는 것은 아니다. application이 loss recovery, congestion/rate control, security, NAT traversal, reordering과 server fan-out을 추가하면 handshake를 아낀 비용보다 상위 protocol 비용이 커질 수 있다. 종단 간 지연 시간, loss tolerance, bandwidth fairness와 운영 복잡성을 TCP·QUIC·다른 transport와 같은 조건에서 비교한다.
