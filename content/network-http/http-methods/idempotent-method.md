...
idempotent method는 **같은 method의 identical 요청을 여러 번 수행했을 때 사용자가 요청한 intended effect on the server가 한 번 수행한 것과 같도록 정의된 method**다. RFC 9110에서 safe method와 PUT, DELETE는 idempotent다. 이것은 응답 상태·timestamp·access log·revision history 같은 관찰 가능한 모든 결과가 매번 byte-for-byte 같아야 한다는 뜻이 아니다. server는 각 요청을 별도로 기록하는 등 요청된 effect 밖의 non-idempotent side effect를 가질 수 있다.

이 성질은 communication 실패 뒤 재시도를 판단할 때 중요하다. 예를 들어 PUT을 전송한 뒤 응답을 읽기 전에 connection이 끊기면 client는 같은 idempotent 요청을 새 connection으로 다시 보낼 수 있다. 반대로 POST처럼 non-idempotent method는 실제 semantics가 반복 안전하다는 별도 지식이 있거나 원래 요청이 적용되지 않았음을 확인할 수 있는 경우가 아니라면 자동 retry 대상으로 일반화하면 안 된다.
