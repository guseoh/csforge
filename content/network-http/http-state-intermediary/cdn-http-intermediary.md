...
CDN은 여러 edge에 HTTP intermediary를 배치해 client 가까이에서 응답을 전달하는 분산 reverse proxy/shared cache다. edge는 요청의 cache key와 HTTP policy를 평가해 fresh hit이면 origin fetch 없이 응답을 반환할 수 있고, miss나 revalidation이 필요하면 origin으로 별도 요청을 보낸다. client-to-edge와 edge-to-origin은 별도 connection이므로 TLS, header, timeout, retry와 forwarding policy도 각각 존재한다.

CDN은 TLS termination, compression, range 처리, signed URL 검증, header rewrite와 edge function을 추가할 수 있다. 이 기능이 origin과 다른 `Content-Encoding`, `Vary`, authorization scope 또는 status를 만들면 cache key와 selected representation이 어긋날 수 있다. purge는 모든 edge의 object를 하나의 원자적 순간에 없앤다는 보장이 아니며, stale serving·in-flight 응답·전파 지연을 포함한 운영 policy가 필요하다. private·personalized 응답은 edge shared cache에 들어가지 않도록 명시적으로 제한한다.
...
canonical content publish와 CDN purge를 하나의 atomic transaction으로 표현하지 않는다. purge 실패를 재시도하고 edge별 상태를 관측하며, 사용자에게 현재 origin·Elasticsearch indexing·CDN cache 상태를 구분해 보여준다. 개인화 API 응답은 공개 curriculum edge object와 별도의 cache scope와 test fixture로 검증한다.
### CDN cache 경계
    요청 → CDN key
              ├─ safe shared variant → edge hit
              └─ personalized/unknown → bypass → origin
purge와 publish는 별도 상태이며 개인화 응답은 shared key와 분리한다.
