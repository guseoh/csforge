...
controller가 `int page`를 요구한다면 `abc`를 int로 바꾸는 과정에서 실패할 수 있습니다. 이 오류는 “page는 1 이상이어야 한다”는 검증보다 앞선 **type conversion/binding 실패**입니다.
...
### rejected value를 log/응답에 그대로 넣지 않는다

password, token, 주민번호 같은 민감한 input도 binding error를 만들 수 있습니다. “어떤 값이 잘못됐는지 보여주자”는 이유로 rejected value를 응답/log에 그대로 넣으면 정보 유출이 생깁니다.
...
두 날짜의 관계처럼 특정 field 하나에만 귀속하기 어려운 요청-level constraint가 있을 수 있습니다.

```text
startDate <= endDate
```

이 경우 `fieldErrors`만 강제하지 않고 global/요청 error로 표현하는 것이 더 자연스럽습니다. 물론 이 관계가 domain invariant라면 DTO 검증이 아니라 domain에서 보호해야 하는지 다시 봅니다.
