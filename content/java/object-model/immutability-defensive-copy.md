...
호출자 names ─────────▶ 원본 mutable List

Team.members ─────────▶ 별도 unmodifiable List snapshot
...
반대로 값 객체 자체가 불변이고 소유권 공유가 안전하다면 모든 계층을 무조건 깊게 복사할 필요는 없습니다. **어느 지점까지 변경 가능성이 존재하는지**를 기준으로 필요한 복사 깊이를 정해야 합니다.
...
#### Q. `final List<T>`이면 불변 객체가 된 것 아닌가요?

아닙니다. `final`은 **그 필드가 다른 List 참조로 재대입되는 것**을 막습니다. 이미 가리키고 있는 List가 가변이라면 원소 추가·삭제는 여전히 가능할 수 있습니다.

불변 객체를 만들려면 외부가 같은 가변 객체를 계속 보유하는지, 내부 컬렉션을 다시 외부에 노출하는지, 원소 객체 자체도 가변인지까지 함께 봐야 합니다.

#### Q. `List.copyOf`를 사용하면 deep copy가 되나요?

그렇게 일반화하면 안 됩니다. `List.copyOf`는 입력 컬렉션의 이후 구조 변경과 분리된 unmodifiable List를 얻는 데 유용하지만, **그 안에 담긴 가변 원소 객체까지 재귀적으로 복사해 주는 것은 아닙니다.**

원소가 불변 값이라면 참조를 안전하게 공유할 수 있고, 원소도 가변이라면 원하는 불변성 수준에 따라 원소 복사나 불변 변환을 추가로 검토해야 합니다.
