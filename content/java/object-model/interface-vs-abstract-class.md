...
또한 구현체가 하나라는 이유만으로 인터페이스가 무의미한 것도 아닙니다. 외부 시스템 경계를 애플리케이션의 의미 있는 계약으로 번역하거나 서로 다른 변경 이유를 분리할 필요가 있다면 구현 개수와 무관하게 interface가 가치 있을 수 있습니다.
...
학습 후는 “다중 상속이 필요하면 인터페이스” 한 문장보다, 인터페이스는 역할·계약 중심이고 추상 클래스는 공통 상태와 기반 구현을 공유하는 클래스 계층에 적합하다고 설명하는 편이 훨씬 좋습니다.

### 면접에서 이렇게 나옵니다

#### Q. 인터페이스와 추상 클래스는 어떤 기준으로 선택하나요?

여러 타입이 같은 **역할·계약**을 제공해야 한다면 interface가 자연스럽고, 하나의 클래스 계층에서 공통 **상태·생성 과정·기반 구현**을 공유해야 한다면 abstract class를 먼저 검토할 수 있습니다.

단순히 “메서드 구현이 있으면 추상 클래스, 없으면 인터페이스”로 나누면 현대 Java의 `default` 메서드나 실제 설계 목적을 놓치게 됩니다.

#### Q. Spring 서비스는 테스트나 DI를 위해 항상 인터페이스를 만들어야 하나요?

아닙니다. Spring은 구체 클래스도 Bean으로 관리하고 주입할 수 있습니다. 인터페이스는 프레임워크 의무가 아니라 **호출자가 의존해야 할 별도 계약이 실제로 존재하는지**를 기준으로 선택해야 합니다.

구현체가 하나뿐이더라도 외부 시스템 경계를 보호하는 의미 있는 계약이라면 가치가 있을 수 있고, 반대로 분리할 이유가 없다면 `Service`와 `ServiceImpl`을 기계적으로 한 쌍 만드는 것은 불필요한 계층이 될 수 있습니다.
