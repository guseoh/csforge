...
단순 기능 재사용
Report ──?──▶ FileUtil
          └─ "Report is a FileUtil"이 자연스럽지 않다면
             상속보다 협력을 먼저 검토
```
...
이때 중요한 질문은 “현실 세계에서 문장으로 is-a라고 말할 수 있는가?” 하나뿐이 아닙니다. **프로그램에서 상위 타입이 약속한 동작과 조건을 하위 타입도 지킬 수 있는가**까지 봐야 합니다.
...
복습할 때도 “상속보다 합성이 좋다”를 외우기보다 **상속은 타입 관계와 구현 결합을 함께 만든다**고 설명하는 편이 정확합니다.

### 면접에서 이렇게 나옵니다

#### Q. 상속을 코드 재사용 목적으로만 사용하면 왜 문제가 될 수 있나요?

상속은 메서드를 재사용하는 기능만 제공하는 것이 아니라 **하위 타입 관계와 상위 구현에 대한 결합**도 함께 만듭니다. 하위 클래스가 protected 상태나 상위 구현 순서에 의존하면 상위 클래스의 내부 변경이 여러 하위 클래스까지 전파될 수 있습니다.

기능만 빌려 쓰는 관계라면 다른 객체를 협력자로 두는 composition이 더 자연스러운지 먼저 검토하는 것이 좋습니다.

#### Q. is-a 관계이면 무조건 상속해도 되나요?

그것만으로는 부족합니다. 이름상 is-a처럼 보여도 하위 타입이 상위 타입의 행동 계약을 지키지 못하면 대체 가능한 subtype이라고 보기 어렵습니다.

따라서 **상위 타입을 기대하는 코드에 하위 객체를 넣어도 의미와 계약이 유지되는가**를 함께 봐야 합니다. 상위 메서드를 하위 타입에서 계속 예외로 막아야 한다면 타입 경계를 다시 설계할 신호일 수 있습니다.
