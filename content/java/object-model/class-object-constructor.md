...
### 생성자가 많아지면 이름이 있는 생성 API도 생각할 수 있다

생성 방식이 여러 개가 되면 매개변수만 보고 의도를 구분하기 어려울 수 있습니다.

```java
Order order = new Order(1L, 2, false);
```

`false`가 무엇을 뜻하는지 호출부만 보고 이해하기 어렵다면 static factory처럼 이름으로 생성 의도를 드러내는 방법도 있습니다.

```java
Order order = Order.draft(1L, 2);
```

이것이 생성자보다 항상 우월하다는 뜻은 아닙니다. 생성 규칙이 단순하고 명확하면 생성자가 가장 직접적일 수 있고, 생성 경로의 의미를 이름으로 구분해야 할 때 별도 creation API가 유용할 수 있습니다.
...
#### Q. 생성자는 단순히 필드에 값을 넣는 곳인가요?

그보다 넓게 보는 편이 좋습니다. 생성자는 **새 객체가 외부에 사용 가능한 상태로 나가기 전에 필수 값과 invariant를 만족시키는 생성 경계**가 될 수 있습니다.

모든 비즈니스 규칙을 생성자에 몰아넣는다는 뜻은 아니지만, 객체가 존재하려면 반드시 만족해야 하는 조건은 생성 과정에서 보장할 수 있는지 먼저 검토하는 것이 좋습니다.

#### Q. 기본 생성자는 항상 자동으로 만들어지나요?

아닙니다. 클래스에 생성자를 하나도 선언하지 않았을 때 컴파일러가 기본 생성자를 제공할 수 있습니다. 개발자가 생성자를 하나라도 선언하면 매개변수 없는 생성자가 자동으로 함께 추가되는 것은 아닙니다.

JPA 같은 프레임워크가 별도의 no-arg constructor 규칙을 요구할 수 있지만, 그것은 Java 언어의 기본 생성자 규칙과 구분해야 합니다.
