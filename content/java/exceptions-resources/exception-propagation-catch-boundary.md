...
![예외가 책임 있는 경계까지 전파되는 흐름](/learning/java/exception-boundaries.svg)
...
이 코드는 대체 처리라는 실제 복구 행동이 있습니다. 반대로 예외를 잡고 빈 목록이나 `null`을 반환하면 호출자가 실패와 정상적인 빈 결과를 구분하지 못할 수 있습니다.
