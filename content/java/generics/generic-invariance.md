...
#### Q. `Integer`가 `Number`의 하위 타입인데 왜 `List<Integer>`는 `List<Number>`의 하위 타입이 아닌가요?

`List<Number>`라는 참조는 `Double` 같은 다른 `Number`도 추가할 수 있어야 합니다. 만약 실제 `List<Integer>`를 그 타입으로 허용하면 Integer 목록 안에 Double을 쓸 수 있게 되어 원래 타입 계약이 깨집니다. 그래서 Java의 일반 제네릭 타입은 불공변으로 두고, 읽기 또는 쓰기 방향을 넓혀야 할 때 bounded wildcard로 관계를 표현합니다.

#### Q. 배열은 공변인데 제네릭은 왜 다르게 설계되어 있나요?

참조형 배열은 `String[]`을 `Object[]`로 볼 수 있지만 실제 component type을 런타임에 알고 있어 잘못된 저장을 `ArrayStoreException`으로 막습니다. 제네릭은 같은 위험을 많은 경우 컴파일 단계에서 차단하도록 기본 타입 관계를 불공변으로 둡니다. 두 방식은 오류를 발견하는 시점과 런타임 타입 정보 모델이 다릅니다.
