...
`EnumSet`의 구현 세부를 특정 bit layout으로 고정해 설명할 필요는 없습니다. 중요한 보장은 원소가 해당 enum 타입의 상수로 제한되고, `allOf`, `noneOf`, `complementOf`처럼 가능한 값 전체를 기준으로 집합을 조작할 수 있다는 점입니다. 권한 정책을 다룰 때는 빈 집합과 모든 권한 집합을 구분하고, 변경 가능한 집합을 외부에 그대로 노출하지 않는 ownership 경계도 함께 확인합니다.
