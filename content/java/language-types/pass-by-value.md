...
아닙니다. Java의 메서드 인자 전달은 **항상 pass-by-value**입니다. 참조 타입을 넘길 때도 객체 자체나 호출자의 변수 슬롯을 넘기는 것이 아니라, 호출자 변수가 가진 **참조 값이 매개변수에 복사**됩니다.

복사된 두 참조 값이 같은 가변 객체를 가리키기 때문에 메서드 안에서 객체 상태를 바꾸면 호출자에게도 보일 수 있습니다. 반대로 매개변수를 다른 객체로 재대입해도 호출자의 변수는 바뀌지 않는다는 점까지 설명하면 규칙과 결과를 함께 보여줄 수 있습니다.

#### Q. pass-by-value인데 메서드 안에서 객체 상태가 바뀌는 이유는 무엇인가요?

pass-by-value가 보장하는 것은 **매개변수라는 새 변수에 값이 복사된다**는 점입니다. 그 복사된 값이 같은 가변 객체를 가리키는 참조 값이라면, 매개변수를 통해 그 객체의 상태를 변경할 수 있습니다.

따라서 `value = new Member()`처럼 **참조 값을 재대입하는 동작**과 `value.name = "lee"`처럼 **공유 객체의 상태를 변경하는 동작**을 구분해야 합니다.
