...
먼저 성공 가능성을 판단한 뒤 상태를 변경하면 실패 경로에서 기존 invariant를 유지하기 쉽습니다. 이런 성질을 **실패 atomicity** 관점에서 볼 수 있습니다. 반드시 모든 메서드가 완벽한 트랜잭션처럼 동작해야 한다는 뜻은 아니지만, 실패했을 때 어떤 변경이 남는지는 계약의 일부입니다.
...
검증 API 하나를 썼다고 도메인 invariant 전체가 해결되는 것은 아닙니다. 타입 자체로 더 잘 표현할 수 있는 값이라면 `Email`, `PositiveQuantity` 같은 value object를 고려할 수도 있고, 단순한 내부 값이면 명시적인 검증 method가 더 직접적일 수 있습니다.
...
#### Q. 매개변수 검증은 Controller와 Domain 중 어디에서 해야 하나요?

한쪽에 전부 몰아넣는 문제가 아닙니다. **HTTP 요청 형식이나 필수 필드처럼 표현 경계가 아는 조건은 API에서**, 현재 객체 상태와 상태 전이처럼 도메인이 소유하는 invariant는 Domain에서 지키는 편이 자연스럽습니다.

같은 business rule을 Controller와 Domain에 기계적으로 중복하면 정책이 여러 곳에 퍼질 수 있습니다. 핵심은 검증 횟수가 아니라 어떤 경계가 그 규칙의 의미를 소유하는지입니다.

#### Q. 상태를 바꾼 뒤 예외를 던지면 앞선 변경도 자동으로 취소되나요?

순수 Java 객체에서는 그렇지 않습니다. 예외는 **제어 흐름을 중단할 뿐 이미 수행한 field assignment를 되돌리지 않습니다.** 실패 시 기존 상태를 보존해야 한다면 가능한 조건을 mutation 전에 확인하는 식으로 실행 순서를 설계해야 합니다.

DB transaction rollback은 별도의 framework/transaction 계약입니다. Java exception 자체의 동작과 Spring 같은 transaction manager가 제공하는 rollback 정책을 같은 보장으로 설명하면 안 됩니다.
