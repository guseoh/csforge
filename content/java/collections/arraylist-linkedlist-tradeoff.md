...
#### Q. LinkedList는 중간 삽입이 O(1)인데 왜 ArrayList를 기본 선택으로 많이 쓰나요?

“삽입 자체”와 “삽입 위치를 찾는 전체 연산”을 나눠서 봐야 합니다. 특정 node를 이미 알고 있다면 LinkedList의 연결 변경은 작게 끝날 수 있지만, `add(index, value)`처럼 index로 위치를 찾아야 한다면 탐색 비용이 먼저 듭니다. 반면 많은 실제 List 사용은 순회와 index 접근이 중심이고, node 객체를 따로 할당하지 않는 ArrayList가 더 단순한 기본 선택이 되는 경우가 많습니다. 최종 판단은 실제 workload와 측정 결과로 확인합니다.

#### Q. ArrayList가 빠른 이유를 “메모리에 객체가 연속으로 저장되기 때문”이라고 말해도 되나요?

그렇게 일반화하면 안 됩니다. ArrayList의 backing array에는 원소 참조가 배열 형태로 저장되지만, 각 원소 객체의 물리 메모리 배치까지 Java 언어가 연속으로 보장하지는 않습니다. API가 보장하는 index 접근 특성과 JVM·하드웨어 수준의 locality 설명을 구분해야 합니다.
