...
### heap의 상태와 정렬된 목록은 다르다

`PriorityQueue`는 head를 빠르게 꺼내기 위한 구조입니다. `poll()` 한 번마다 현재 우선순위가 가장 높은 원소를 root에서 제거하고 자식 중 다음 후보를 올리지만, 나머지 배열 영역까지 전체 순서로 정렬해 두지는 않습니다.

```text
offer(5), offer(1), offer(3)
        ↓
peek() = 1       // 지금 꺼낼 head만 보장
iterator()       // 1, 3, 5 순서를 보장하지 않음
poll() → 1 → 3 → 5
```

전체를 정렬된 순서로 관찰해야 한다면 반복해서 `poll`해 복사본을 소모하거나 별도의 sort를 선택해야 합니다. queue를 읽는 행위 자체가 원본을 바꾸어도 되는지까지 호출 계약에 포함하세요.
