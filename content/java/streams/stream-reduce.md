...
특히 parallel stream에서는 입력을 여러 부분 결과로 나눈 뒤 combiner가 이를 합칠 수 있어야 합니다. 뺄셈·문자열 누적처럼 결합 순서에 따라 결과가 달라지는 연산을 무심코 reduce에 넣으면 순차 실행에서는 보이던 결과가 병렬 실행에서 달라질 수 있습니다. identity도 모든 부분 결과의 시작값으로 사용되므로 도메인 값과 항등 원소를 구분해야 합니다.
