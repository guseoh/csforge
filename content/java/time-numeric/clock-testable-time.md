...
이 구분은 테스트에서도 중요합니다. `Clock.fixed`를 넣었다고 만료 정책이 올바르다는 뜻은 아닙니다. 오히려 경계 시각을 정확히 고정할 수 있으므로 `before / equal / after` 각각에서 정책이 의도대로 동작하는지를 명확하게 검증할 수 있게 된 것입니다.
...
이것은 Clock API의 필수 규칙이 아니라 일관된 업무 판단을 만들기 위한 설계 선택입니다. 한 유스케이스에서 "현재"가 하나의 기준 시점을 의미한다면 그 값을 한 번 캡처하는 편이 로그와 테스트에서도 결과를 설명하기 쉽습니다.
...
2. 코드가 `now()`를 직접 호출하는지 `Clock`을 통해 읽는지 봅니다.
...
#### Q. 테스트에서 `Instant.now()`를 직접 호출하지 않고 `Clock`을 주입하는 이유는 무엇인가요?

현재 시각을 외부 입력으로 분리하면 테스트가 실제 실행 시각에 의존하지 않습니다. `Clock.fixed`로 마감 직전·정확한 마감 시각·직후를 재현할 수 있어 시간 경계 정책을 결정적으로 검증할 수 있습니다.

#### Q. 한 요청에서 `Instant.now(clock)`을 여러 번 호출하면 항상 문제인가요?

항상 문제인 것은 아니지만 여러 판단이 하나의 업무상 "현재"를 공유해야 한다면 호출 사이에 시간이 지나 서로 다른 기준을 볼 수 있습니다. 그런 유스케이스에서는 시작 시각을 한 번 캡처해 같은 `Instant`를 전달하는 편이 판단과 로그의 일관성을 높입니다.
