...
`LocalDateTime.atZone(zone)`처럼 local date-time을 zone에 결합하는 API에는 gap과 overlap을 해석하는 기본 규칙이 있습니다. 그러나 **API가 어떤 값을 만들어 준다는 사실과 그 값이 예약 정책상 옳다는 판단은 별개**입니다. 사용자가 실제로 의도한 시각을 보존해야 하는 기능이라면 전환 구간을 별도로 검증할 필요가 있습니다.
...
또 timezone 규칙은 현실 세계의 법·정책 변화에 따라 갱신될 수 있습니다. Java API를 사용한다고 해서 미래의 지역 규칙이 영원히 고정되는 것은 아닙니다. `ZoneId`가 지역을 식별하고 실제 계산에는 runtime이 제공하는 zone rules가 사용되므로, 장기 예약 시스템이라면 어떤 zone 정보를 보관하고 규칙 변경을 언제 다시 반영할지까지 운영 정책으로 볼 수 있습니다.
...
`ZoneOffset`은 특정 시점의 UTC 차이를 나타내고 `ZoneId`는 지역의 날짜별 시간대 규칙을 식별한다고 설명하면 됩니다. DST 전환에서는 존재하지 않는 local time인 gap과 두 번 나타나는 overlap이 생길 수 있으므로 예약이나 반복 일정에서는 Java API의 기본 보정에만 의존하지 말고 업무 정책을 명시해야 합니다.

### 면접에서 이렇게 나옵니다

#### Q. `ZoneOffset`과 `ZoneId`는 무엇이 다른가요?

`ZoneOffset`은 UTC와의 고정된 차이를 나타내지만 `ZoneId`는 `Europe/Paris`처럼 지역의 시간대 규칙을 식별합니다. 장기 일정에서는 같은 지역도 날짜에 따라 offset이 달라질 수 있으므로 현재 offset 하나만 저장해서는 지역의 미래 규칙을 보존할 수 없습니다.

#### Q. DST overlap에서 `02:30`이라는 값만 저장하면 왜 모호할 수 있나요?

시계를 뒤로 돌리는 전환에서는 같은 local 시각이 서로 다른 두 offset으로 두 번 나타날 수 있습니다. 따라서 local date-time만으로는 하나의 `Instant`를 정할 수 없고, 어느 offset을 선택할지 또는 사용자의 원래 zone 정책을 어떻게 보존할지 결정해야 합니다.
