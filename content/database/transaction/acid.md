...
### Durability: commit 성공 이후의 데이터 보존 계약

COMMIT이 성공했다고 응답한 transaction의 변경은 crash 후에도 복구되어야 합니다. PostgreSQL은 WAL 같은 메커니즘으로 이 계약을 구현합니다. 다만 replica까지 즉시 반영되었는지, 외부 시스템도 같은 상태인지까지 durability 한 단어가 보장하는 것은 아닙니다.
