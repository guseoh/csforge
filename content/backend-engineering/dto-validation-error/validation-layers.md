...
title: 검증 계층
summary: Parsing, API 요청, Domain invariant, DB constraint는 서로 다른 실패를 서로 다른 경계에서 막는다.
level: 2
status: PUBLISHED
displayOrder: 20
references:
...
검증은 한곳에만 두면 깔끔해 보이지만 실제로는 **서로 다른 실패를 서로 다른 경계가 소유**합니다. 같은 규칙을 중복한다는 이유만으로 모든 검증을 Controller나 Domain 한곳에 몰면 우회 경로가 생기거나 사용자 피드백이 늦어집니다.
...
| 요청 검증 | 제목 길이, 필수 field | API 입력 계약 위반                 |
