...
서버 메서드 이름을 바꾸는 것은 내부 리팩터링일 수 있지만, 응답 필드를 삭제하거나 의미를 바꾸는 것은 이미 배포된 클라이언트를 깨뜨릴 수 있는 계약 변경이다. API evolution은 코드가 컴파일되는지가 아니라 **구버전 소비자가 새 서버와 계속 통신할 수 있는가**를 본다.
